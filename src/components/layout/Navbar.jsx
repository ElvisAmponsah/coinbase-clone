            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-16">
                    ...
                    <div className="hidden lg:flex items-center gap-1">
                        {navLinks.map((link) => (
                            <Link
                                key={link.name}
                                to={link.path}
                                className={`px-4 py-2 text-sm font-medium rounded-full transition-colors duration-200 ${isActive(link.path)
                                        ? 'text-cb-blue bg-blue-50'
                                        : 'text-cb-gray-600 hover:text-cb-dark hover:bg-cb-gray-50'
                                    }`}
                            >
                                {link.name}
                            </Link>
                    ...
                    <button
                        ...
                        className="lg:hidden p-2 rounded-lg text-cb-gray-600 hover:bg-cb-gray-100 transition-colors"
                    >
                ...
                {isMenuOpen && (
                    <div className="lg:hidden border-t border-cb-gray-200 py-4 animate-in">
                        <div className="flex flex-col gap-1">
                            {navLinks.map((link) => (
                                <Link
                                    ...
                                    className={`px-4 py-3 text-base font-medium rounded-xl transition-colors duration-200 ${isActive(link.path)
                                            ? 'text-cb-blue bg-blue-50'
                                            : 'text-cb-gray-600 hover:text-cb-dark hover:bg-cb-gray-50'
                                        }`}
                                >
                        ...
                        <div className="flex flex-col gap-2 mt-4 pt-4 border-t border-cb-gray-200">
                            <Button variant="ghost" to="/signin" onClick={() => setIsMenuOpen(false)}>
                                Sign in
                            </Button>
                            <Button variant="primary" to="/signup" onClick={() => setIsMenuOpen(false)}>
                                Sign up
                            </Button>
                        </div>
                    </div>
