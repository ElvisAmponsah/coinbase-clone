...
import { getAssetById, cryptoAssets } from '../data/cryptoData'
...
const AssetDetail = () => {
    ...
    const stats = [
        ...
        { label: 'All-time High', value: `$${(asset.price * 1.8).toLocaleString(undefined, { maximumFractionDigits: 2 })}` },
    ]
                        ...
                        <div className="bg-white rounded-2xl border border-cb-gray-200 p-8 mb-6">
                            <div className="flex items-center gap-4 mb-6">
                                ...
                                <div>
                                    ...
                                    <p className="text-cb-gray-500">{asset.symbol}</p>
                                </div>
                            </div>
                            ...
                            <div className="h-64 bg-cb-gray-50 rounded-xl overflow-hidden">
                                <svg viewBox="0 0 600 250" className="w-full h-full" preserveAspectRatio="none">
                                    <defs>
                                        <linearGradient id={`detailGrad-${asset.id}`} x1="0%" y1="0%" x2="0%" y2="100%">
                                            <stop offset="0%" style={{ stopColor: asset.color, stopOpacity: 0.2 }} />
                                            <stop offset="100%" style={{ stopColor: asset.color, stopOpacity: 0 }} />
                                        </linearGradient>
                                    </defs>
                                    <polygon
                                        fill={`url(#detailGrad-${asset.id})`}
                                        points={`${generateDetailChart(isPositive)},600,250,0,250`}
                                    />
                                    <polyline
                                        fill="none"
                                        stroke={asset.color}
                                        ...
                                        points={generateDetailChart(isPositive)}
                                    />
                                </svg>
                            </div>
                            ...
                            <div className="flex gap-2 mt-4">
                                {['1H', '1D', '1W', '1M', '1Y', 'All'].map((period) => (
                                    <button
                                        key={period}
                                        className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${period === '1D'
                                                ? 'bg-cb-blue text-white'
                                                : 'text-cb-gray-500 hover:bg-cb-gray-100'
                                            }`}
                                    >
                                        {period}
                                    </button>
                            ...
                            <div className="grid sm:grid-cols-2 gap-6">
                                {stats.map((stat) => (
                                    <div key={stat.label} className="flex justify-between items-center py-3 border-b border-cb-gray-100">
                                        ...
                                        <span className="font-semibold text-cb-dark">{stat.value}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-2xl border border-cb-gray-200 p-6 sticky top-24">
                            ...
                            <div className="mb-4">
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        type="number"
                                        placeholder="0.00"
                                        className="w-full pl-8 pr-4 py-3 border border-cb-gray-200 rounded-xl text-cb-dark focus:outline-none focus:ring-2 focus:ring-cb-blue focus:border-transparent"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="flex gap-2 mb-4">
                                {['$25', '$50', '$100', '$500'].map((amount) => (
                                    <button
                                        key={amount}
                                        className="flex-1 py-2 text-sm font-medium text-cb-gray-600 bg-cb-gray-50 rounded-lg hover:bg-cb-gray-100 transition-colors"
                                    >
                                        {amount}
                                    </button>
                                ))}
                            ...
                            <div className="space-y-3">
                                <Button variant="primary" className="w-full" to="/signup">
                                    Buy {asset.symbol}
                                </Button>
                            ...
                            <p className="text-xs text-cb-gray-400 text-center mt-4">
                                Sign up to start trading {asset.name}
                            </p>
                        </div>
                    </div>
                </div>
...
// Generate a realistic-looking chart
